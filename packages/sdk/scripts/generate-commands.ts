...
export function generateCommands() {
  ...
  // Generate runtime TS for SDK bundling
  const generatedDir = path.join(srcDir, "generated");
  if (!fs.existsSync(generatedDir)) {
    fs.mkdirSync(generatedDir, { recursive: true });
  }
  const runtimeTSCode = generateRuntimeTS(namespaceCommands, namespaceEvents);
  const runtimeTSOutputPath = path.join(generatedDir, "runtime.ts");
  fs.writeFileSync(runtimeTSOutputPath, runtimeTSCode);
  console.log(
