 ...
 * 
 * Usage: ts-node scripts/generate-i18n.ts
 *        (or via pnpm run codegen)
 */
 ...
 */
function generateTypeCode(keys: FlattenedKey[]): string {
  const lines: string[] = [];

  lines.push("/**");
  lines.push(" * AUTO-GENERATED FILE - DO NOT EDIT");
  lines.push(" *");
  lines.push(" * Common translations from the Eden SDK.");
  lines.push(" * Generated by packages/sdk/scripts/generate-i18n.ts");
  lines.push(" * Run 'pnpm run codegen' to regenerate.");
  lines.push(" */");
  lines.push("");
  lines.push("/**");
  lines.push(" * Type-safe mapping of common translation keys to their argument requirements.");
  lines.push(" * - `void` means no interpolation arguments needed");
  lines.push(" * - `{ argName: string | number }` means arguments are required");
  lines.push(" */");
...
export function generateI18nTypes() {
  console.log("🔍 Generating i18n types from SDK locale files...");
  ...
  // Extract the exported object using regex (simpler than full TS parsing for this case)
  const objectMatch = localeContent.match(/export\s+const\s+\w+\s*=\s*(\{[\s\S]*\});?\s*$/);
  if (!objectMatch) {
    console.error("❌ Could not parse locale file");
    return;
  }
  
  // Parse the object (safe since we control the format)
  let localeObj: LocaleObject;
  try {
    // Convert to valid JSON by replacing property names without quotes
    // and handling the object literal syntax
  ...
  const typeCode = generateTypeCode(flatKeys);
