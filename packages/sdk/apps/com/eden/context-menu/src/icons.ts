/**
 * Curated subset of Feather icons for context menus.
 *
 * Icons are stored as raw SVG inner content (without wrapper) for minimal bundle size.
 * The wrapper is added at render time with controlled attributes.
 *
...
export const CONTEXT_MENU_ICONS: Record<string, string> = {
  ...
  "rotate-ccw": `<polyline points="1 4 1 10 7 10"></polyline><path d="M3.51 15a9 9 0 1 0 2.12-9.36L1 10"></path>`,
...
/**
 * Resolve an icon name to its SVG inner content.
 * Returns null if the icon name is not in the allowlist.
 */
export function resolveIconSvg(name: string): string | null {
  return CONTEXT_MENU_ICONS[name] ?? null;
}
