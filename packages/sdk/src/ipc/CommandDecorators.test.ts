...
describe("CommandDecorators", () => {
  it("stores namespace, events, and handler metadata", () => {
    ...
    expect(Reflect.getMetadata("eden:namespace", ProcessManager)).toBe("process");
    expect(Reflect.getMetadata("eden:events", ProcessManager)).toBe("ProcessEvents");
    expect(
      Reflect.getMetadata("eden:handler:permission", ProcessManager.prototype, "handleLaunch"),
    ).toBe("manage");
    expect(
      Reflect.getMetadata("eden:handler:grant", ProcessManager.prototype, "handleLaunch"),
