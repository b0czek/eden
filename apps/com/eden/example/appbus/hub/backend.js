...
worker.appBus.exposeService('chat-relay', (connection, { appId: clientAppId }) => {
  ...
  connection.handle('join', ({ clientId, nickname }) => {
    ...
    broadcastToOthers(clientAppId, 'client-joined', { clientId: clientAppId, nickname });
  ...
  connection.handle('send-message', ({ clientId, message }) => {
    ...
    console.log(`[Hub] Broadcasting message from ${client.nickname}: ${message}`);
