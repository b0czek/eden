...
export function setupWindowDragging(
    ...
    const startDrag = (e: MouseEvent | TouchEvent) => {
        ...
        console.log('[Eden Frame] currentBounds before refresh:', currentBoundsRef.current);
        ...
        if (initialBounds && initialBounds.x !== undefined) {
            ...
            console.log('[Eden Frame] Refreshed currentBounds from edenFrame._internal.bounds:', currentBoundsRef.current);
    ...
    const endDrag = (e: MouseEvent | TouchEvent) => {
        ...
        if (e.type.startsWith('touch') && (e as TouchEvent).touches && (e as TouchEvent).touches.length > 0) {
            return;
        }
        ...
        console.log('[Eden Frame] Drag ended, final currentBounds:', currentBoundsRef.current);
        isDragging = false;
        ...
        if (isTouch && currentBoundsRef.current) {
            ...
            console.log('[Eden Frame] Updated edenFrame._internal.bounds after touch drag:', window.edenFrame?._internal.bounds);
        }
    ...
    document.addEventListener('touchmove', moveDrag as EventListener, { passive: false, capture: true });
