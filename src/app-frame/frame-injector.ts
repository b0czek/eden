...
import { createOverlay, injectOverlay, setupDarkMode, setTitle, getAppName } from './ui-builder.js';
import { setupCloseButton, setupMinimizeButton, setupToggleModeButton } from './button-handlers.js';
...
(function () {
    ...
    if (!window.edenFrame) {
        window.edenFrame = {
            setTitle: (title: string) => { /* will be set later */ },
            _internal: {
                injected: false,
                config: {},
                currentMode: 'tiled',
    ...
    const currentBoundsRef: { current: { x: number; y: number; width: number; height: number } | null } = { current: null };
    ...
    const setupFloatingWindowControls = (): void => {
        ...
        if (initialBounds && initialBounds.x !== undefined) {
            ...
            console.log('[Eden Frame] Initialized bounds from backend:', currentBoundsRef.current);
            ...
            console.log('[Eden Frame] Initialized bounds from config:', currentBoundsRef.current);
        }
