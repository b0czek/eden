 ...
 */
export function getScreenCoords(e: MouseEvent | TouchEvent): { x: number; y: number } {
...
/**
 * Wait for a global API to be available
 * @param apiName - Name of the API (e.g., 'appAPI', 'edenAPI')
 * @param callback - Called when API is available
 * @param interval - Check interval in ms (default: 100)
 */
export function waitForAPI(apiName: string, callback: (api: any) => void, interval: number = 100): void {
    const checkInterval = setInterval(() => {
        if ((window as any)[apiName]) {
            clearInterval(checkInterval);
            callback((window as any)[apiName]);
        }
    }, interval);
}
